...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # this goes 1 to 5 from top to bottom
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # goes from left to right
matrix2
...
matrix1[4, ] #4 refers to row, and column is on the right. If you dont specify column, then only gives all values on row
matrix1[ , 3] #gies all values on column
matrix1[c(12, 14)] #gives 12 and 14
matrix1[c(12:14)] #gives 12,13 and 14
matrix1[2:4, 1:3] #subset of rows and columns, has rows 2 to 4 and columns 1 to 3
...
```{r DataExercise-1a}
low_temp <- c(56,58,60,60,62,62,64,64,62,60)
high_temp <- c(82,83,84,79,71,78,82,85,84,82)
```
...
low_C <- celsius(low_temp)
high_C <- celsius(high_temp)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
df_temp <- data.frame("LowF"=low_temp,"HighF"= high_temp,low_C,high_C)
df_temp
```
...
#with as date you need to specify the data format
...
#Ex1
str_today <- "2023-feb-7" #string today
