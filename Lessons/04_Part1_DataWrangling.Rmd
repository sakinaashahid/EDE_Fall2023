...
#can only compare character with character or numeric with numeric in rows

# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]
#filtering rows that have a 0 on column depth
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #using or because it can either be paul lake or peter lake
...
#begin with changing integer to date and then filter

```
Question: Why don't we filter using row numbers?

> Answer: those numbers will change once its filtered
...
# : everything in between sampledate and temperature C
