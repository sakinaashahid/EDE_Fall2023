...
ggplot(PeterPaul.chem.nutrients,
       aes(x=factor(PeterPaul.chem.nutrients$month, levels=1:12, labels=month.abb))) +
  geom_bar() + scale_x_discrete(name="month",drop=FALSE)
...
plot <- ggplot(PeterPaul.chem.nutrients, aes(x = daynum, y = temperature_C, color=year4)) +  facet_wrap(vars(lakename), nrow = 2) +
    geom_point()
...
plot <- ggplot(filter(PeterPaul.chem.nutrients,depth==0), aes(x = daynum, y = temperature_C, color=year4)) +  facet_wrap(vars(lakename),nrow=2) + ylab("Temperature")+
    geom_point()
...
plot2 <- ggplot(PeterPaul.chem.nutrients, aes(x=sampledate,y= temperature_C, color=depth))+ geom_point(size=0.5)+scale_color_viridis(option="magma",direction=-1) + scale_x_date(date_breaks = "5 years",date_labels="%Y")
print(plot2)
