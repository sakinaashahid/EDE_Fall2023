...
```{r}
#Exercise 1:
plot1 <-  ggplot(EPAair,aes(x = Ozone,fill=as.factor(Year))) +
  geom_density(alpha=0.5) + scale_fill_manual(values=c("darkgreen","darkblue")) +
  labs(x="Ozone AQI Value", y= "Density",fill="")
print(plot1)

plot2 <-  ggplot(EPAair,aes(y=Ozone)) + geom_boxplot()+ stat_summary(geom="text",fun=max,vjust=-1,size=4,label =c ("b","a"))+
  labs (x="","ozone")
print(plot2)
