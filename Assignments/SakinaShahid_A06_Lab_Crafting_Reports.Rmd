...
```{r boxplot ozone 2018,fig.cap="Monthly Ozone 2018",fig.align='left',fig.height=3,warning=FALSE,echo=FALSE}
  
ozone_2018 <- epa_data %>% 
  drop_na(Ozone) %>% 
  select(Ozone, Month, Year)%>%
  filter(Year==2018)

                          

ozone2018plot <- ggplot(ozone_2018,
aes(x=factor(ozone_2018$Month, levels=1:12, labels=month.abb), 
...
```{r boxplot ozone 2019,fig.cap="Monthly Ozone 2019",fig.align='left',fig.height=3,warning=FALSE,echo=FALSE}
...
ozone2019plot <- ggplot(ozone_2019,
aes(x=factor(ozone_2019$Month, levels=1:12, labels=month.abb), 
  y = Ozone)) + geom_boxplot() + 
  scale_x_discrete(name="Month",drop=FALSE)+
  ylab(expression (paste("Concentration(", mu, g/m^{3},")")))+
