...
theme <- theme_linedraw(base_size = 13) +
  theme(axis.text = element_text
    (color = "black"),plot.title = element_text(color="black",size=14),
...
y = tp_ug, color=lakename))+
  ...
  ylab("Phosphorus")+
  labs(color="Lake Name")+
  ggtitle("Phosphorus concentrations versus Month for Paul and Peter Lake")+
  theme + theme(legend.position="none")
...
y = tn_ug, color=lakename))+
  ...
  ylab("Phosphate")+
  labs(color="Lake Name")+
  ggtitle("Phosphate concentrations versus Month for Paul and Peter Lake")+
  theme +
...
The median Phosphate concentrations fall within the range of the IQR for each lake, making the difference relatively small. There is observably more Phosphate in Peter Lake in August and the outliers across all months are larger compared to Paul Lake, indicating more culmulative concentrations of Phosphate in Peter Lake. 
...
needle_plot <- ggplot(needles.df,aes(x=collectDate,
                                     y=dryMass,
                                     color=nlcdClass))+ geom_point(size=0.9)+xlab("Date")+ylab("Dry Mass (mg)")+
  labs(color="Class")+
